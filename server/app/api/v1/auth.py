...
async def check_email(
    ...
    try:
        ...
        return {
            ...
            "exists": exists,
            "message": (
                "이미 등록된 이메일입니다" if exists else "사용 가능한 이메일입니다"
            ),
        }
    ...
    try:
        ...
        if not user:
            user = await user_service.create_user_google(
                UserCreateGoogle(
                    google_id=sub, email=email, name=name, profile_image_url=picture
                )
            )
        ...
        raise HTTPException(
            status_code=400, detail=f"Google token verify failed: {str(e)}"
        )
